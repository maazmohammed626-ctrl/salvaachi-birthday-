...
function setupImageFallbacks() {
  document.querySelectorAll("img").forEach((image) => {
    image.addEventListener("error", () => {
      image.hidden = true;
      image.closest("figure, button, .drawer-art, .flower-card, .photo-card")?.classList.add("image-missing");
    });
  });
}
...
function flowerImage(flower) {
  return `<img class="noto-flower" src="${flower.asset}" alt="" loading="lazy" />`;
}
